...
| 签名公钥指数 e 的值 |65537|
| 签名公钥模数 N 的前 16 位十六进制 | 00:d5:ae:c7:07:97:4f:15:0c:85:08:d1:81:70:7d|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） |这是一条重要消息，需要签名保护，发送者：王凤娥，学号：2024010034 |
| 密文文件 encrypted_message.bin 大小（字节） | 256|
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |3e45e732fc28cc78262597ae6a8b16f41f9777d9d647c9b94cc3b3029ce389c1 |
...
> 答：三大保证：完整性、身份认证（鉴别）、不可否认性
完整性
含义：消息在传输、存储过程中没有被篡改、增删、修改任意比特。
实现：先对原文哈希，签名哈希值；验证时重新计算文件哈希，与签名解密出的哈希对比，不一致则文件被改动。
身份认证（数据源鉴别）
含义：能确认签名者是持有对应私钥的合法主体，不是他人伪造。
实现：只有签名者拥有签名私钥，用私钥签名后，任何人只能用配套公钥验证；公钥验证通过，说明签名由对应私钥持有者生成，确认发送方身份。
不可否认性
含义：签名者事后无法否认自己签署过该文件。
实现：私钥唯一归属签名人，公钥公开可第三方核验，第三方可出具证明，签名行为无法抵赖。
结合实验实现：实验中用私钥对文件哈希签名；接收方用公钥解密签名得到原始哈希，本地重新哈希文件比对，保证完整性；公钥验证通过确认是对应私钥持有者发送（身份认证）；私钥仅本人持有，签署记录可留存，实现不可否认。
...
> 答：RSA 明文长度有限制
RSA 加密 / 签名输入不能超过密钥模长。若文件很大（几 MB/GB），无法直接完整签名；哈希输出固定短长度（如 SHA256 为 32 字节），适配 RSA 输入限制。
提升签名效率
大文件直接 RSA 签名运算极慢；哈希是单向快速运算，仅对短小哈希值做 RSA 模幂运算，大幅降低计算耗时。
缩小签名数据体积
直接对大文件签名会输出超长数据；哈希定长，最终生成的签名长度固定（等于密钥模长），便于存储、传输。
安全加固，避免明文结构漏洞
直接签名原始明文易遭受 RSA 结构攻击；哈希破坏明文结构，消除基于消息格式的伪造漏洞。
...
> 答：RSA 签名输出长度 = 密钥模长（二进制位数）÷ 8
2048 位 RSA：2048 ÷ 8 = 256 字节，因此 signature.bin 大小为 256 字节。
...
> 答：抗原像性（单向性）
定义：给定哈希输出 h，无法在可行时间内找到任意原文 m，使得 Hash (m)=h。
漏洞危害：攻击者拿到哈希值可反推出原始文件，伪造原始消息。
抗第二原像性（弱抗碰撞）
定义：给定消息 m1，无法找到另一条不同消息 m2（m1≠m2），满足 Hash (m1)=Hash (m2)。
漏洞危害：攻击者拿到合法文件，构造另一份内容完全不同但哈希相同的文件，替换后签名验证依然通过，篡改不被发现。
抗强碰撞性（抗碰撞）
定义：无法找到两条完全不同消息 m1、m2，m1≠m2 且 Hash (m1)=Hash (m2)。
漏洞危害：攻击者提前构造一对哈希相同的文件，一份正常发送获取签名，再替换为恶意文件，验证哈希匹配，伪造签名文件。
...
> 答：淘汰原因：SHA-1 已被证明存在可计算碰撞，不满足抗碰撞性，无法保障数字签名安全；NIST 等标准已禁止用于签名、证书等安全场景，仅允许老旧归档读取。
2017 Google SHA-1 碰撞攻击影响：攻击者可构造两份不同文档，哈希值完全一致。
攻击举例：
攻击者制作两份 PDF，一份是合法合同 A，一份是恶意欠条合同 B，二者 SHA-1 哈希完全相同。将 A 发给用户签名，拿到 A 的数字签名；之后用恶意 B 替换原文件。校验时重新计算 SHA-1 哈希和签名解密出的哈希一致，验证程序判定签名合法，受害者被迫承认恶意文件有效。
...
> 答：签名密钥对（用于数字签名）
签名私钥：签名操作，对文件哈希加密生成签名；仅持有者保管，绝不公开。
签名公钥：验证签名操作，解密签名得到原始哈希，公开分发。
加密密钥对（用于消息加密）
加密公钥：加密明文，任何人可用公钥加密消息，只有对应私钥能解密。
加密私钥：解密密文，接收方解密收到的加密数据。
攻击者获取公钥后的能力
能做：验证签名、加密消息发给密钥持有者；
不能做：伪造合法签名、解密使用该公钥加密的数据、推导私钥。
...
> 答：核心作用区分
消息加密：目标保密性，防止第三方窃听读取消息，只保证内容不泄露，无法证明发送方身份、防篡改、防抵赖。
数字签名：目标完整性、身份认证、不可否认，不保证消息保密，任何人持有公钥均可查看原始消息。
流程对比（加密→哈希→签名→验证→解密）
加密：使用接收方公钥加密原文，只有接收私钥可读；解决窃听问题。
签名：使用发送方私钥对哈希签名，所有人用发送方公钥核验；解决篡改、冒充、抵赖问题。
不能用加密替代签名的原因
加密无法实现不可否认：加密用接收方公钥，任何人都能加密，无法定位发送者；签名唯一绑定发送方私钥。
加密无法直接校验完整性，缺少哈希比对校验逻辑。
加密是双向可逆，单纯加密不能证明文件未被修改；签名依靠哈希比对检测篡改。
加密密钥使用对象完全相反，混淆会造成安全失效。
...
> 答：不可否认性定义
签署文件后，签名者无法否认自己签署过该文件；第三方权威机构可核验证据，具备法律效力。
数字签名可提供、MAC 不能的原因
数字签名：非对称密码，私钥仅签名人独有，公钥全网公开、第三方可独立核验。一旦签名验证通过，证据客观唯一指向私钥持有人，无法抵赖。
MAC（消息认证码）：基于对称密钥，通信双方共享同一密钥。密钥双方都持有，接收方可自行伪造 MAC 值，事后无法区分是发送方伪造还是接收方篡改，不存在第三方可信证据，无法实现抵赖追责。
...
> 答：软件 / 安装包代码签名
场景：Windows 软件、安卓 APK、开源程序发布。
解决问题：防止安装包被第三方植入病毒木马篡改；确认安装包来自官方开发者，用户校验签名辨别恶意修改程序。
电子邮件签名（S/MIME）
场景：企业商务邮件、涉密公文邮件。
解决问题：防止邮件内容中途被篡改、防止他人冒充领导 / 客户发送虚假邮件；邮件发送者无法否认发送过该邮件。
电子合同、电子签章
场景：线上租房、金融贷款、政务电子文书。
解决问题：替代手写签字，具备法律效应；防止合同条款私自修改，签约人不可否认签署行为。
补充可选场景：CA 数字证书、区块链交易签名、固件升级包校验。
...
> 答：数字签名本身无法自带可信时间：签名仅绑定文件与私钥，不含权威时间戳；私钥持有者可随时重新签名，修改有效时间。
验证签名真实时间方案：引入可信时间戳服务 TSA。
流程：用户对文件签名后，将签名发送给第三方可信 TSA 机构；TSA 使用自身私钥，将标准权威时间和用户签名绑定生成时间戳签名，返还用户。
事后核验：一年后核验时，同时校验用户原始签名 + TSA 时间戳签名；TSA 是中立第三方，时间由国家授时源校准，用户无法篡改时间，证明文件确切签署日期。
...
> 答：1024 位 RSA
安全性：已不安全，现有算力 + 数论算法可在短期破解模数；NIST 早在 2013 年淘汰，禁止用于签名、证书、业务系统。
性能：运算速度快，但安全风险极高，实验与生产均不推荐。
2048 位 RSA（实验选用）
安全性：当前通用安全标准，抗已知暴力、分解攻击，满足政务、互联网、教学实验安全需求，预计未来 10 年内安全。
性能平衡：模幂运算速度适中，签名 / 验证耗时低，硬件、软件兼容性好；兼顾安全与运行效率，是教学、商用主流选择。
...
> 答：单一层面：单纯 Alice 签名、Bob 再签 Alice 签名本身，无意义
仅对他人签名二次嵌套签名，无法增加完整性、身份校验能力，只会增大数据体积、增加计算开销，无安全增益。
多重签名有意义的场景（两类）
层级授权签名（机构多级审批）
场景：企业财务付款单据，员工 Alice 起草文件签名，部门主管 Bob 复核后对 Alice 的签名 + 原文整体二次签名。作用：证明经过两级审批，缺少任意一层签名则单据无效，实现分级权责校验。
多方联合多重签名（多人共同授权）
场景：区块链多签钱包、多方合作合同。需要所有参与方分别对同一文件独立签名，任意一方缺失签名则交易 / 合同不生效，实现多方共同确认、权责共担。
公证归档签名
第三方公证处对用户原始签名文档加盖公证签名，作为法律存证，增强司法采信效力。
